...
const task71 = products.slice(0, 5).map(p => p.name);
console.log(task71);

const task72 = products.slice(-5).filter(p => p.price > 10000);
console.log(task72);

const task73 = products.filter(p => p.brand === "Apple").map((p) => ({
    name: p.name,
    price: p.price
}));
console.log(task73);

const task74 = products
    .filter(p => p.rating >= 4.5)
    .map(p => p.name)
    .sort((a, b) => a.localeCompare(b));
console.log(task74);

const task75 = products
    .filter(p => p.stock > 0)
